...
- Run ` + "`segmentstream run`" + ` after changing ` + "`segmentstream.yml`" + `.
...
Dagster/dbt environment at http://localhost:3000, and runs the SegmentStream
materialization. The first run can take a few minutes while Docker downloads
and builds the local environment; later runs should be faster.
...
- Use ` + "`segmentstream run`" + ` after changing ` + "`segmentstream.yml`" + `; it regenerates the local runtime and runs materialization.
