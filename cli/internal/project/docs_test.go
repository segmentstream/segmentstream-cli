...
func TestEnsureProjectReadmeCreatesReadme(t *testing.T) {
	...
	for _, want := range []string{
		...
		"segmentstream source contracts --type conversion_events",
...
func TestEnsureAgentGuideCreatesGuide(t *testing.T) {
	...
	if !strings.Contains(string(data), "segmentstream source scaffold") {
		t.Fatalf("agent guide does not mention source scaffold:\n%s", string(data))
	}
}
