...
class SegmentStreamSource:
    ...
    conversion_events_model_name: str
...
def dbt_vars(
    ...
    data = {
        ...
        "segmentstream_conversion_event_sources": conversion_event_source_vars(sources),
...
def conversion_event_source_vars(sources: list[SegmentStreamSource]) -> list[dict[str, str]]:
    return [
        {
            "name": source.name,
            "package_name": source.package_name,
            "conversion_events_model_name": source.conversion_events_model_name,
        }
        for source in sources
        if source.contract_type == "conversion_events"
    ]
...
def parse_sources(config: dict) -> list[SegmentStreamSource]:
    ...
    for raw_source in raw_sources:
        ...
        if contract_type not in {"events", "conversion_events", "identity_keys"}:
            raise RuntimeError(
        ...
        conversion_events_model_name = model_name if contract_type == "conversion_events" else "conversion_events"
        ...
        sources.append(
            SegmentStreamSource(
                ...
                conversion_events_model_name=conversion_events_model_name,
