...
func newWarehouseCommand(out io.Writer, credentialStore credentials.Store, registry warehouse.Registry) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "warehouse",
	...
	cmd.AddCommand(newWarehouseAuthCommand(out, credentialStore))
...
func newWarehouseAuthCommand(out io.Writer, credentialStore credentials.Store) *cobra.Command {
	options := warehouseAuthOptions{Name: "default-bigquery"}
	cmd := &cobra.Command{
		Use:   "auth --service-account-key <path>",
		Short: "Store a warehouse service-account credential",
		Long: "Store a BigQuery service-account key for the warehouse selected in segmentstream.yml.\n\n" +
			"The key is copied to ~/.segmentstream/bigquery/<name>.json with restrictive\n" +
			"file permissions. No credential material is written to segmentstream.yml.",
		Args: cobra.NoArgs,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if config.Warehouse.Type != "bigquery" {
				return fmt.Errorf("unsupported warehouse.type %q", config.Warehouse.Type)
			}
